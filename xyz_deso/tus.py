...
def resume(file_obj,
    ...
    while data:
        res=_upload_chunk(data, offset, file_endpoint, headers=headers)
        total_sent += len(data)
    ...
    if not _is_seekable(file_obj):
        ...
        res= _upload_chunk('', offset, file_endpoint, headers=headers)
    return res
...
def _upload_chunk(data, offset, file_endpoint, headers=None):
    ...
    if response.status_code != 204:
        print(response.status_code, response.text, response.headers, sep='\n')
